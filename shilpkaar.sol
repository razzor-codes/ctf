...
contract Shilpkaar{
    ...
    function unlock(bytes32 _name, bytes32 _password)external {
        ...
        require(uint256(garbageAddress) > 2**153 + garbageNonce && uint256(garbageAddress) <= 2**160 - ((garbageDivisor-1) - garbageNonce), "Problem With garbageAddress");//garbageNonce
        require(gateKey3<uint256(uint64(-1))*49/100 && gateKey4<uint256(uint64(-1))*51/100 && probablyPrime(gateKey3) && probablyPrime(gateKey4), "Problem with gateKey 3 and 4");
        uint256 masterKey = uint256(gateKey1)+uint256(gateKey2)+uint256(gateKey3)+uint256(gateKey4)+uint256(gateKey5);
        require(masterKey > (2**65+2**56) && masterKey < (2**65+2**56+2**16), "Problem with addition of master key");
        ...
        require(shilpkaar[msg.sender]);
        require(block.timestamp>=timeToRoll[msg.sender]);
    ...
    returns(uint256)
{
    ...
    function updateSeed(uint256 _garbageDivisor, uint256 _garbageMultiplier) external{
        require(owner == msg.sender);
    ...
    function updateNonce(uint256 _garbageNonce) external{
        require(owner == msg.sender);
